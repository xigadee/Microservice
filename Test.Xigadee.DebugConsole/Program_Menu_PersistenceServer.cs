    ...
    static partial class Program
    {
        static Lazy<ConsoleMenu> sPersistenceMenuServer = new Lazy<ConsoleMenu>(
            ...
            , (m, o) =>
                {
                    ...
                    var result = sContext.Server.Persistence.Create(
           ...
           , new ConsoleOption("Read entity"
               , (m, o) =>
               {
                   var result = sContext.Server.Persistence.Read(sContext.Testid, new RepositorySettings() { WaitTime = TimeSpan.FromMinutes(5) }).Result;
           ...
           , new ConsoleOption("Read entity by reference"
               , (m, o) =>
               {
                   var result = sContext.Server.Persistence.ReadByRef("email", "paul@hotmail.com", new RepositorySettings() { WaitTime = TimeSpan.FromMinutes(5) }).Result;
                   PersistenceLog(m, "Read", result.IsSuccess);
               }
               , enabled: (m, o) => sContext.PersistenceStatus() == 2
               )
           , new ConsoleOption("Update entity"
               , (m, o) =>
               {
                   var result = sContext.Server.Persistence.Update(
                       new MondayMorningBlues()
                       {
                           Id = sContext.Testid,
                           ContentId = sContext.Testid,
           ...
           , new ConsoleOption("Delete entity"
               , (m, o) =>
               {
                   var result = sContext.Server.Persistence.Delete(sContext.Testid, 
           ...
           , new ConsoleOption("Delete entity by reference"
               , (m, o) =>
               {
                   var result = sContext.Server.Persistence.DeleteByRef("email", "paul@hotmail.com", 
                       new RepositorySettings() { WaitTime = TimeSpan.FromMinutes(5), VersionId = sContext.Versionid.ToString() }).Result;
                   PersistenceLog(m, "Delete", result.IsSuccess);
               }
               , enabled: (m, o) => sContext.PersistenceStatus() == 2
